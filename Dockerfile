...
#
# Builds VLights.asi (a Windows x64 DLL) from a Linux container with
# mingw-w64. Nothing needs to be installed on the host besides Docker.
#
#   docker build --target export -o dist .
#
# produces dist/VLights.asi and dist/vlights.ini.
...
WORKDIR /src
COPY CMakeLists.txt VERSION vlights.ini ./
COPY cmake/ cmake/
COPY src/ src/
COPY res/ res/
...
RUN cmake -S . -B build-tests -G Ninja -DCMAKE_BUILD_TYPE=Release -DVLIGHTS_BUILD_TESTS=ON \
...
RUN mkdir -p /out \
 && cp build/VLights.asi vlights.ini /out/ \
 && x86_64-w64-mingw32-objdump -p /out/VLights.asi | grep -E 'DLL Name'
